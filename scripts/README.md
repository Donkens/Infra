...
| `check-proxmox-backup-age.sh` | Mac mini | Kontrollerar ålder och storlek på senaste VM 101/102 backup-filer i `/Users/yasse/InfraBackups/proxmox-dumps/`. Pushar status till Uptime Kuma Push-monitor "Proxmox backup freshness" (ID 16). Token i `~/.config/infra/proxmox-backup-monitor.env` (chmod 600, ej committad). LaunchAgent `com.yasse.proxmox-backup-age-check` kör 05:00. |
