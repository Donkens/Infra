...
## Runtime sudo/admin status

Current Pi admin policy is intentionally simple:

- `pi` is a normal admin user and may use `sudo`.
- The issue #14 day-to-day fixed-wrapper privilege model has been removed.
- Cockpit does not require a separate admin-user split in this repo policy.
- The retained narrow sudoers fragments are backup/audit helpers, not the removed wrapper hardening model:
  - `/etc/sudoers.d/pi-audit-readonly`
  - `/etc/sudoers.d/pi-dns-backup-export`
- The preserved DNS backup export wrapper is `/usr/local/sbin/infra-backup-dns-export`.

Operational consequence:

- Routine administrative commands may be run through normal `sudo` after explicit operator approval.
- Backup/export tooling remains preserved and should not be removed as part of privilege-model cleanup.
