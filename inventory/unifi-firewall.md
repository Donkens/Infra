...
> Last verified: 2026-05-06 CEST — IoT gateway DNS bypass blocked and runtime-verified
...
| `block-iot-gateway-dns-udp` | yes | BLOCK | zone policy | IOT zone `6980de97e060a06b8ef9b613` ANY | Gateway zone IP `192.168.10.1` | UDP `53` | `69fb7a221bc6e72d277782ad` | Added 2026-05-06 after client validation showed gateway DNS answered from IoT. |
| `block-iot-gateway-dns-tcp` | yes | BLOCK | zone policy | IOT zone `6980de97e060a06b8ef9b613` ANY | Gateway zone IP `192.168.10.1` | TCP `53` | `69fb7a221bc6e72d277782b0` | TCP companion to IoT gateway DNS block. |
...
| `allow-termix-to-pi-ssh` | **yes** | ALLOW | zone policy | Docker VM / Termix `192.168.30.10` in Server zone `69f7de611bc6e72d2776b75b` | Pi DNS `192.168.1.55` in Internal zone `677d9959ed22014620a6a981` | TCP `22` | `69fbb2601bc6e72d27779357` | Added 2026-05-06 for Termix Pi target onboarding. Minimal single-host SSH exception, `ip_version: IPV4`, `create_allow_respond: true`, index 10004. No HAOS or broad Server VLAN access. |
...
| `Proxmox` | **no** | ALLOW | zone policy | Docker VM `192.168.30.10` in Server zone `69f7de611bc6e72d2776b75b` | Opti `192.168.1.60` in Internal zone `677d9959ed22014620a6a981` | TCP/UDP `8006` | `69f8cc521bc6e72d2776ec01` | Disabled. Created 2026-05-04 to validate Docker VM → Proxmox UI path (port 8006) during Phase 1A work. Left disabled after validation. `ip_version: BOTH`, `create_allow_respond: true`, `connection_state_type: ALL`, index 10003. Delete via UniFi UI when convenient. |
...
- IoT client DNS bypass is verified blocked from iPhone on `UniFi IOT`: `@192.168.10.1` and `@1.1.1.1` timed out after adding `block-iot-gateway-dns-udp/tcp`.
...
## IoT VLAN 10 DNS bypass closure — completed 2026-05-06

Runtime-tested from iPhone on `UniFi IOT` using Deep Dig.

| Test | Expected | Result |
|---|---|---|
| IoT → Pi DNS `@192.168.1.55 pi.home.lan A` | `192.168.1.55` | ✅ PASS |
| IoT → Pi DNS `@192.168.1.55 cloudflare.com A` | public A records | ✅ PASS |
| IoT → WAN DNS `@1.1.1.1 cloudflare.com A` | TIMEOUT | ✅ BLOCKED |
| IoT → gateway DNS `@192.168.10.1 cloudflare.com A` | TIMEOUT | ✅ BLOCKED |

Before `block-iot-gateway-dns-udp/tcp` was added, `@192.168.10.1` answered
`cloudflare.com`, confirming the gateway DNS bypass gap.
...
| `allow-termix-to-pi-ssh` | Server → Internal | IP `192.168.30.10` (Docker VM / Termix) | IP `192.168.1.55` (Pi DNS) | TCP 22 | ALLOW | `69fbb2601bc6e72d27779357` | ✅ Live 2026-05-06 — minimal Termix Pi target SSH exception, `ip_version: IPV4`, `create_allow_respond: true` |
...
| `Proxmox` | Server → Internal | IP `192.168.30.10` (Docker VM) | IP `192.168.1.60` (Opti) | TCP/UDP 8006 | ALLOW | `69f8cc521bc6e72d2776ec01` | ⛔ Disabled — temp validation rule created 2026-05-04; delete via UniFi UI when convenient |
