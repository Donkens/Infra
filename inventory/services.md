...
| Uptime Kuma | 3001 | HTTP behind Caddy HTTP/HTTPS | live | `louislam/uptime-kuma:1.23.15`. Intern nät only, ingen host-port. Via Caddy → `kuma.home.lan`. HTTPS live via Caddy `tls internal`; HTTP monitor remains active. Phase 1C-C4 attempted HTTPS monitor move and rolled back because the Kuma container itself lacks Caddy CA trust. Admin-lösenord satt. Proxmox-monitor pausad. |
