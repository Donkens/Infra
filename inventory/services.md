...
| Uptime Kuma | 3001 | HTTP behind Caddy HTTP/HTTPS | live | `louislam/uptime-kuma:1.23.15`. Intern nät only, ingen host-port. Via Caddy → `kuma.home.lan`. Phase 1C-C4a: Docker/Caddy monitors for `proxy`, `kuma`, and `dockge` now use HTTPS with per-monitor Caddy `tlsCa` (`auth_method=mtls`, `ignore_tls=0`). No Dozzle monitor exists yet. Admin-lösenord satt. Proxmox-monitor pausad. |
